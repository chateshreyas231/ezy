    ...
    <View style={{ flex: 1 }}>
      <View style={{ flexDirection: 'row', padding: 16, gap: 8, borderBottomWidth: 1, borderColor: '#eee' }}>
        <Button
          ...
          color={activeTab === 'messages' ? Theme.colors.accent : Theme.colors.textTertiary}
        />
        <Button
          title="Intent"
          onPress={() => setActiveTab('intent')}
          color={activeTab === 'intent' ? Theme.colors.accent : Theme.colors.textTertiary}
        />
        <Button
          title="Compensation"
          onPress={() => setActiveTab('compensation')}
          color={activeTab === 'compensation' ? Theme.colors.accent : Theme.colors.textTertiary}
        />
      </View>
        ...
        <ScrollView style={{ flex: 1, padding: 20 }}>
          ...
          {intents.map((intent) => (
            <View key={intent.id} style={{ padding: 16, marginBottom: 12, backgroundColor: '#f5f5f5', borderRadius: 8 }}>
              ...
              <Text style={{ fontSize: 12, color: Theme.colors.textTertiary, marginTop: 4 }}>
                {new Date(intent.created_at).toLocaleString()}
              </Text>
            </View>
          ))}

          <View style={{ marginTop: 20, padding: 16, backgroundColor: Theme.colors.surfaceElevated, borderRadius: 8 }}>
            ...
            <TextInput
              ...
              multiline
              style={{ borderWidth: 1, borderColor: Theme.colors.border, padding: 12, borderRadius: 8, marginBottom: 12, minHeight: 80 }}
            />
            <TextInput
              value={offerAmount}
              ...
              style={{ borderWidth: 1, borderColor: Theme.colors.border, padding: 12, borderRadius: 8, marginBottom: 12 }}
            />
            <Button
      ...
      {activeTab === 'compensation' && (
        <ScrollView style={{ flex: 1, padding: 20 }}>
          ...
          {compensations.map((comp) => (
            <View key={comp.id} style={{ padding: 16, marginBottom: 12, backgroundColor: Theme.colors.surface, borderRadius: 8 }}>
