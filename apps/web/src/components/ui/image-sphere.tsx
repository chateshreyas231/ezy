...
export interface ImageData {
    ...
    title?: string;
    description?: string;
    // Add extra fields for agent data integration
    agentId?: string;
}
...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
    ...
    // Filter out the selected image from the rotating sphere
    const sphereImages = React.useMemo(() => {
        return selectedId ? images.filter(img => img.id !== selectedId && img.agentId !== selectedId) : images;
    }, [images, selectedId]);

    const centeredImage = React.useMemo(() => {
        return selectedId ? images.find(img => img.id === selectedId || img.agentId === selectedId) : null;
    }, [images, selectedId]);
    ...
    const generateSpherePositions = useCallback((): SphericalPosition[] => {
        ...
        const imageCount = sphereImages.length;
        ...
        return positions;
    }, [sphereImages.length, actualSphereRadius]);
    ...
    const renderSpotlightModal = () => {
        if (!selectedImage || onImageClick) return null; // Don't show modal if custom click handler exists
    ...
    if (!isMounted) {
        return (
            <div
                className="bg-gray-100/5 rounded-lg animate-pulse flex items-center justify-center"
    ...
    if (!images.length) {
        return (
            <div
                className="bg-gray-50/5 rounded-lg border-2 border-dashed border-gray-300/20 flex items-center justify-center"
                style={{ width: containerSize, height: containerSize }}
            >
      ...
      `}</style>
            ...
            >
                <div className="relative w-full h-full" style={{ zIndex: 10 }}>
                    ...
                    {centeredImage && (
                        <div
                            className="absolute top-1/2 left-1/2 rounded-full shadow-[0_0_50px_rgba(var(--primary),0.5)] z-[2000] overflow-hidden border-4 border-primary/50 animate-pulse"
                            style={{
                                width: `${baseImageSize * 1.8}px`,
                                height: `${baseImageSize * 1.8}px`,
                                transform: 'translate(-50%, -50%)',
                            }}
                        >
                            <img
                                src={centeredImage.src}
                                alt={centeredImage.alt}
                                className="w-full h-full object-cover"
                            />
                        </div>
                    )}
