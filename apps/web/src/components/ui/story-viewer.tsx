...
function StoryThumbnail({ stories, username, viewedIndices, onClick, className }: StoryThumbnailProps) {
  ...
  const thumbnailImage = React.useMemo(() => {
    for (let i = stories.length - 1; i >= 0; i--) {
      if (stories[i].type === "image") return stories[i].src;
    }
    return null;
  }, [stories]);
...
function StoryViewerModal({
  ...
  const handleVideoTimeUpdate = React.useCallback((currentTime: number, videoDuration: number) => {
    if (videoDuration > 0) setProgress((currentTime / videoDuration) * 100);
  }, []);
