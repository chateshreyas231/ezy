...
const VerticalCutReveal = forwardRef<VerticalCutRevealRef, TextProps>(
  (
    ...
    const splitIntoCharacters = (content: string): string[] => {
      if (typeof Intl !== "undefined" && "Segmenter" in Intl) {
        const segmenter = new Intl.Segmenter("en", { granularity: "grapheme" });
        return Array.from(segmenter.segment(content), ({ segment }) => segment);
      }
    ...
    const getStaggerDelay = useCallback(
      (index: number) => {
        ...
        if (staggerFrom === "random") {
          const pseudoRandomIndex = (index * 7 + 3) % Math.max(total, 1);
          return Math.abs(pseudoRandomIndex - index) * staggerDuration;
        }
              ...
              {wordObj.characters.map((char, charIndex) => (
                <span className={cn(elementLevelClassName, "whitespace-pre-wrap relative")} key={charIndex}>
                  <motion.span
                    ...
                    variants={variants}
                    onAnimationComplete={
                      wordIndex === renderWords.length - 1 && charIndex === wordObj.characters.length - 1 ? onComplete : undefined
                    }
