import * as React from "react";
import { motion } from "framer-motion";
import { cn } from "@/lib/utils";
import { CircleHelp } from "lucide-react";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";

type SubStat = {
  value: string | number;
  label: string;
  subLabel: string;
};

type AvailabilityBar = {
  level: number;
};

interface FreelancerStatsCardProps {
  title: string;
  timeFrame: string;
  metricLabel?: string;
  earnings: {
    amount: number;
    change: number;
    changePeriod: string;
    changeDisplay?: string;
  };
  subStats: [SubStat, SubStat];
  ranking: {
    place: string;
    category: string;
    icon?: React.ReactNode;
  };
  availability: {
    title: string;
    bars: AvailabilityBar[];
    label: string;
    helpText?: string;
  };
  className?: string;
}

export const FreelancerStatsCard = React.forwardRef<
  HTMLDivElement,
  FreelancerStatsCardProps
>(({ title, timeFrame, metricLabel = "Earnings", earnings, subStats, ranking, availability, className }, ref) => {
  const currencyFormatter = new Intl.NumberFormat("en-US", {
    style: "currency",
    currency: "USD",
  });

  const containerVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.05,
      },
    },
  };

  const barVariants = {
    hidden: { height: "0%", opacity: 0 },
    visible: {
      height: "100%",
      opacity: 1,
      transition: { duration: 0.5, ease: "easeOut" },
    },
  };

  const barColors = ["#3b82f6", "#4f46e5", "#a855f7", "#d946ef", "#ec4899", "#ef4444"];

  return (
    <div
      ref={ref}
      className={cn(
        "flex w-full max-w-sm flex-col gap-6 rounded-2xl border bg-card p-6 font-sans text-card-foreground shadow-lg",
        className,
      )}
      aria-labelledby="stats-card-title"
    >
      <header className="flex items-center justify-between">
        <h2 id="stats-card-title" className="text-xl font-bold">{title}</h2>
        <div className="rounded-md bg-muted px-3 py-1 text-sm font-medium text-muted-foreground">
          {timeFrame}
        </div>
      </header>

      <section aria-label={metricLabel}>
        <p className="text-sm text-muted-foreground">{metricLabel}</p>
        <h3 className="mt-1 text-5xl font-bold tracking-tighter">
          {currencyFormatter.format(earnings.amount)}
        </h3>
        <p
          className={cn(
            "mt-2 text-sm font-semibold",
            earnings.change >= 0 ? "text-green-500" : "text-red-500",
          )}
        >
          {earnings.changeDisplay ? earnings.changeDisplay : (
            <>
              {earnings.change >= 0 ? "+" : ""}
              {currencyFormatter.format(earnings.change)} {earnings.changePeriod}
            </>
          )}
        </p>
      </section>

      <section className="grid grid-cols-2 gap-4" aria-label="Projects and Clients">
        {subStats.map((stat, index) => (
          <div key={index} className="rounded-lg bg-muted p-4">
            <p className="text-2xl font-bold">
              {stat.value}{" "}
              <span className="text-base font-normal text-muted-foreground">{stat.label}</span>
            </p>
            <p className="mt-1 text-xs text-muted-foreground">{stat.subLabel}</p>
          </div>
        ))}
      </section>

      <section
        className="flex items-center justify-between rounded-lg bg-primary-foreground p-4 text-primary"
        aria-label={`Ranking: ${ranking.place}`}
      >
        <div>
          <h4 className="text-xl font-bold">{ranking.place}</h4>
          <p className="text-sm text-primary/80">{ranking.category}</p>
        </div>
        {ranking.icon && <div aria-hidden="true">{ranking.icon}</div>}
      </section>

      <section aria-labelledby="availability-title">
        <div className="flex items-center gap-2">
          <h4 id="availability-title" className="text-md font-semibold">{availability.title}</h4>
          {availability.helpText && (
            <TooltipProvider delayDuration={120}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    type="button"
                    className="inline-flex items-center text-muted-foreground hover:text-foreground"
                    aria-label={`${availability.title} explanation`}
                  >
                    <CircleHelp className="h-4 w-4" />
                  </button>
                </TooltipTrigger>
                <TooltipContent className="max-w-xs text-xs leading-relaxed">
                  {availability.helpText}
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
        </div>
        <motion.div
          className="mt-3 flex h-12 items-end gap-1"
          variants={containerVariants}
          initial="hidden"
          animate="visible"
          aria-label="Availability chart"
        >
          {availability.bars.map((bar, index) => {
            const colorIndex = Math.floor((index / availability.bars.length) * (barColors.length - 1));
            const color = bar.level > 0 ? barColors[colorIndex] : "var(--muted)";

            return (
              <div
                key={index}
                className="flex h-full w-full items-end rounded-sm"
                style={{ backgroundColor: bar.level > 0 ? "transparent" : "var(--muted)" }}
              >
                <motion.div
                  className="w-full rounded-sm"
                  style={{
                    height: `${bar.level * 100}%`,
                    backgroundColor: color,
                  }}
                  variants={barVariants}
                />
              </div>
            );
          })}
        </motion.div>
        <p className="mt-2 text-xs text-muted-foreground">{availability.label}</p>
      </section>
    </div>
  );
});

FreelancerStatsCard.displayName = "FreelancerStatsCard";
