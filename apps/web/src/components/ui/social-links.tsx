...
const AnimatedSocialLinks = React.forwardRef<HTMLDivElement, AnimatedSocialLinksProps>(
  ({ socials, className, ...props }, ref) => {
    ...
    return (
      <div ref={ref} className={cn("flex items-center justify-center gap-0", className)} {...props}>
        {socials.map((social, index) => (
          ...
          >
            <span className="block text-sm text-white whitespace-nowrap">{social.name}</span>
            <AnimatePresence>
